...
export const fetchProducts = createAsyncThunk('products/get',async () => {
    const req = await fetch('https://dashboard-api-vq7r.onrender.com/api/products')
    const data = await req.json()
    return data
})

export const removeProduct = createAsyncThunk('product/detele', async (id: string) => {
    const req = await fetch(`https://dashboard-api-vq7r.onrender.com/api/products/${id}`,{
        method: 'DELETE'
    })
    const res = req.json()
    return res
...
export const editProduct = createAsyncThunk('product/put',async (data: Product) => {
    const req = await fetch(`https://dashboard-api-vq7r.onrender.com/api/products/${data.id}`,{
        method: 'PUT',
        headers: {
